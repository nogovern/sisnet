      ...
      <table class="table" style="border:2px solid #CCC;">
        <tbody>
          ...
          <tr>
            ...
            <td class="col-sm-3">수리업체:
<?php
  echo gs2_location_name($op->work_location);
?>
            </td>
            <td class="col-sm-3">수량: 
...
?>
    ...
    <div class="col-md-12">
      <a class="btn btn-default" href="<?=base_url() . 'work/repair'?>"><i class="fa fa-list"></i> 리스트</a>
<?php if($op->status < '3'): ?>
      ...
      <button id="btn_open_register_item" type="button" class="btn btn-warning">장비등록</button>
<?php endif; ?>

<?php if($op->type == '502' && $op->status == '2'): ?> 
...
$this->view('common/modal_item_register');     // 장비등록 사용
...
$(document).ready(function(){
  ...
  // 장비등록
  $("#btn_open_register_item").click(function(){
    $("#modal_item_register").modal('show');
  });
  ...
  $("#btn_op_accept_complete").click(function(){
    if(!confirm("수리-승인 업무를 완료 하시겠습니까?")) {
      return false;
    }
  ...
  $("#btn_op_send_complete").click(function(){
    if(!confirm("수리 업무를 완료합니다.\n완료 시 실제 재고량이 변경됩니다.\n계속 하시겠습니까?")){
      return false;
    }

    $.get( _base_url + "work/repair/complete/" + operation.id, function( data ) {
      alert( "업무 완료!" );
      gs2_console(data);
    });
  });
  ...
  $("#btn_cancel_request").click(function(){
    ...
    gs2_cancel_operation(_base_url + "work/repair");
  });
